    ...
    try:
        ...
        try:
            ...
            if not people:
                ...
                people = []  # Ensure people is a list even if None is returned
        ...
        if include_linkedin and people:
            try:
                ...
                if linkedin_urls:
                    ...
                    # Process LinkedIn profiles with proper error handling
                    try:
                        ...
                        if linkedin_profiles and isinstance(linkedin_profiles, list):
                            ...
                            logger.warning(f"No valid LinkedIn profiles returned for search {request_id}")
                    except asyncio.TimeoutError:
                        logger.error(f"LinkedIn scraping timed out for search {request_id}")
                        # Continue with existing people data
                    except Exception as scrape_error:
                        logger.error(f"Error during LinkedIn scraping for search {request_id}: {str(scrape_error)}")
                        # Continue with existing people data
                else:
        ...
        # Update the search status to failed only if we're still processing
        if processing_state["is_processing"]:
            processing_state["is_processing"] = False
            try:
                search_data = get_search_from_database(request_id)
