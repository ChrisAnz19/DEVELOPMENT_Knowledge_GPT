...
class DemoSearchGenerator:
    def __init__(self):
        self.search_categories = [
            ...
            # Weird and unconventional use cases
            "personal_real_estate", "luxury_home_buyers", "vacation_property", 
            "investment_property", "commercial_property", "land_acquisition",
            "yacht_purchases", "private_jet_buyers", "art_collectors", "wine_collectors",
            "classic_car_enthusiasts", "luxury_watch_buyers", "jewelry_collectors",
            "antique_hunters", "rare_book_collectors", "sports_memorabilia",
            "cryptocurrency_whales", "nft_collectors", "domain_investors",
            "patent_trolls", "trademark_squatters", "copyright_holders",
            "talent_scouts", "headhunter_hunters", "recruiter_recruiters",
            "consultant_consultants", "advisor_advisors", "mentor_mentors",
            "coach_coaches", "trainer_trainers", "speaker_bookers",
            "event_planners_seeking_planners", "wedding_vendors", "party_planners",
            "personal_chefs", "private_tutors", "nanny_services", "elder_care",
            "pet_services", "house_sitters", "personal_assistants", "concierge_services",
            "lifestyle_managers", "wealth_concierges", "family_office_staff",
            "private_security", "bodyguards", "private_investigators", "forensic_accountants",
            "divorce_attorneys", "estate_planners", "tax_shelters", "offshore_banking",
            "citizenship_by_investment", "golden_visa", "residency_planning", "tax_optimization",
            "political_donors", "lobbyists", "influence_peddlers", "access_brokers",
            "insider_traders", "whistleblowers", "corporate_spies", "industrial_espionage",
            "reputation_management", "crisis_actors", "astroturfing", "sock_puppets",
            "social_media_manipulators", "influencer_fraud", "fake_reviews", "seo_spam"
        ]
        ...
        self.behavioral_signals = [
            ...
            # Weird and unconventional behavioral signals
            "house hunting", "yacht shopping", "private jet browsing", "art collecting",
            "wine investing", "watch collecting", "car collecting", "jewelry shopping",
            "vacation home searching", "investment property hunting", "land acquisition",
            "citizenship shopping", "passport collecting", "residency planning", "tax fleeing",
            "divorce planning", "prenup negotiating", "custody fighting", "alimony dodging",
            "offshore banking", "shell company creating", "tax sheltering", "money laundering",
            "reputation scrubbing", "crisis managing", "scandal covering", "whistleblowing",
            "insider trading", "market manipulating", "pump and dumping", "short selling",
            "patent trolling", "trademark squatting", "domain parking", "copyright claiming",
            "influence peddling", "access brokering", "lobbying", "bribing",
            "headhunter hunting", "recruiter recruiting", "talent scouting", "poaching",
            "consultant consulting", "advisor advising", "mentor mentoring", "coaching coaches",
            "networking networkers", "connecting connectors", "introducing introducers",
            "matchmaking", "deal brokering", "transaction facilitating", "relationship building",
            "social climbing", "status seeking", "clout chasing", "influence buying",
            "reputation building", "personal branding", "thought leadership", "expertise positioning",
            "speaking circuit", "conference hopping", "panel participating", "award hunting",
            "media courting", "pr spinning", "narrative controlling", "story planting",
            "astroturfing", "sock puppeting", "review farming", "seo gaming",
            "algorithm hacking", "growth hacking", "viral engineering", "meme creating",
            "trend surfing", "bandwagon jumping", "fomo exploiting", "hype building"
        ]
...
Generate 1 unique, realistic search query. Be wildly creative and vary the approach, industry focus, geographic scope, company stage, and behavioral indicators. Return only the search query text, nothing else."""
    ...
    def _get_fallback_search(self, category: str, industry: str, location: str) -> Dict[str, Any]:
        ...
        fallback_queries = [
            ...
            # Weird and unconventional searches
            f"Ultra-high-net-worth individuals {behavioral_signal} luxury real estate in {location}",
            f"Crypto whales looking to buy physical assets in {location}",
            f"Tech executives {behavioral_signal} vacation homes in {location}",
            f"Private equity partners shopping for yachts over $50M",
            f"Hedge fund managers collecting vintage cars in {location}",
            f"Family office principals {behavioral_signal} art investments",
            f"Sovereign wealth fund CIOs buying rare wine collections",
            f"Investment committee members {behavioral_signal} citizenship by investment",
            f"Board members seeking golden visa programs in {location}",
            f"Executives planning tax residency moves from {location}",
            f"Wealth managers whose clients are {behavioral_signal} offshore structures",
            f"Private bankers serving clients {behavioral_signal} shell companies",
            f"Tax advisors helping clients with {behavioral_signal} strategies",
            f"Estate planners working with families {behavioral_signal} succession",
            f"Divorce attorneys representing executives {behavioral_signal} asset protection",
            f"Forensic accountants investigating {industry} executives {behavioral_signal}",
            f"Private investigators tracking {industry} insiders {behavioral_signal}",
            f"Reputation management firms helping executives {behavioral_signal} crisis cleanup",
            f"Crisis communications experts managing {industry} scandals in {location}",
            f"Headhunters who are themselves {behavioral_signal} new opportunities",
            f"Executive recruiters looking for other recruiters {behavioral_signal}",
            f"Talent acquisition leaders {behavioral_signal} recruiting consultants",
            f"HR executives seeking advisors who are {behavioral_signal}",
            f"Consultants looking for other consultants {behavioral_signal} partnerships",
            f"Advisory board members {behavioral_signal} their own advisors",
            f"Mentors seeking mentors for their own {behavioral_signal} journey",
            f"Coaches who need coaching on {behavioral_signal} strategies",
            f"Speakers looking for speaker bureaus {behavioral_signal} bookings",
            f"Event planners {behavioral_signal} other event planners for partnerships",
            f"Wedding planners seeking luxury vendors {behavioral_signal} exclusivity",
            f"Concierge services targeting clients {behavioral_signal} lifestyle management",
            f"Personal assistants to executives {behavioral_signal} career advancement",
            f"Private chefs serving families {behavioral_signal} culinary experiences",
            f"Yacht crew recruiters {behavioral_signal} ultra-luxury positions",
            f"Private jet brokers targeting clients {behavioral_signal} aircraft ownership",
            f"Luxury real estate agents whose clients are {behavioral_signal} trophy properties",
            f"Art dealers targeting collectors {behavioral_signal} blue-chip pieces",
            f"Wine investment advisors serving clients {behavioral_signal} cellar building",
            f"Classic car dealers targeting enthusiasts {behavioral_signal} rare models",
            f"Jewelry appraisers working with collectors {behavioral_signal} authentication",
            f"Antique dealers serving clients {behavioral_signal} museum-quality pieces",
            f"Rare book dealers targeting bibliophiles {behavioral_signal} first editions",
            f"Sports memorabilia experts serving collectors {behavioral_signal} game-worn items",
            f"Cryptocurrency OTC traders serving whales {behavioral_signal} large blocks",
            f"NFT advisors helping collectors {behavioral_signal} blue-chip projects",
            f"Domain investors {behavioral_signal} premium .com acquisitions",
            f"Patent brokers targeting inventors {behavioral_signal} monetization",
            f"Trademark attorneys serving brands {behavioral_signal} protection strategies",
            f"Copyright lawyers helping creators {behavioral_signal} IP enforcement",
            f"Lobbyists representing {industry} interests {behavioral_signal} policy changes",
            f"Government relations experts helping {industry} with {behavioral_signal} compliance",
            f"Political consultants advising candidates {behavioral_signal} {industry} support",
            f"Campaign finance lawyers helping donors {behavioral_signal} contribution strategies",
            f"Ethics advisors helping executives navigate {behavioral_signal} conflicts",
            f"Compliance officers at {industry} companies {behavioral_signal} regulatory changes"
